# this is version downloaded from https://github.com/kauralasoo/eigenMT/blob/master/eigenMT.py
# code was translated to Python 3 with necessary updates, such as the usage of map with list, f-string formatting, and a few other tweaks to ensure compatibility.
import os
import sys
import fileinput
import argparse
...
def make_phepos_dict(POS_fh, CHROM):
    ...
    with open(POS_fh) as POS:
        ...
        for line in POS:
            line = line.rstrip().split()
            if line[1] == CHROM:
                pos_array = np.array(line[2:4], dtype=float)
...
def make_gen_dict(GEN_fh, pos_dict):
    ...
    with open(GEN_fh) as GEN:
        ...
        for line in GEN:  # Go through each line of the genotype matrix and add line to gen_dict
            line = line.rstrip().split()
            snp = pos_dict[line[0]]
            genos = np.array(line[1:], dtype=str)
            genos[genos == 'NA'] = -1
            genos = np.array(genos, dtype=float)
...
def make_test_dict(QTL_fh, gen_dict, genpos_dict, phepos_dict, cis_dist):
    ...
    '''
    with open(QTL_fh) as QTL:
        header = QTL.readline().rstrip().split()
        # find the column with the p-value
        pvalIndex = None
        for key in ['p-value', 'p.value', 'pvalue']:
            if key in header:
                pvalIndex = header.index(key)
                break
        if pvalIndex is None:
            ...
            if line[0] in genpos_dict:
                ...
                if snp in gen_dict and gene in phepos_dict:
                    ...
                    if distance <= cis_dist:
                        pval = float(line[pvalIndex])
                        if gene not in test_dict:
                            test_dict[gene] = {
                                'snps': [snp], 'best_snp': snp, 'pval': pval, 'line': '\t'.join(line)}
                        else:
...
def make_test_dict_external(QTL_fh, gen_dict, genpos_dict, phepos_dict, cis_dist):
            ...
            if line[0] in genpos_dict:
                ...
                if snp in gen_dict and gene in phepos_dict:
                    ...
                    if distance <= cis_dist:
                        ...
                        if gene not in test_dict:
                            test_dict[gene] = {
                                'best_snp': snp, 'pval': pval, 'line': '\t'.join(line)}
                        else:
                            if pval < test_dict[gene]['pval']:
                                test_dict[gene]['best_snp'] = snp
                                test_dict[gene]['pval'] = pval
    ...
    for gene in test_dict:
        ...
        is_in_cis_start = np.abs(snps - phepos[0]) <= cis_dist
        is_in_cis_end = np.abs(snps - phepos[1]) <= cis_dist
...
def bf_eigen_windows(test_dict, gen_dict, phepos_dict, OUT_fh, input_header, var_thresh, window):
    ...
    '''
    with open(OUT_fh, 'w') as OUT:
        OUT.write(input_header + '\tBF\tTESTS\tlenthSNPs\n')

        counter = 1.0
        genes = list(test_dict.keys())
        numgenes = len(genes)
        TSSs = [phepos_dict[gene][0] for gene in genes]
        zipped = list(zip(TSSs, genes))
        zipped.sort()
        TSSs, genes = zip(*zipped)
        for gene in genes:
            perc = (100 * counter / numgenes)
            if (counter % 100) == 0:
                print(f"{counter} out of {numgenes} completed ({round(perc, 3)}%)")
            ...
            while start < M:
                ...
                genotypes = [gen_dict[snp] for snp in snps_window if snp in gen_dict]
                ...
                if stop > M:
                    stop = M
            OUT.write(test_dict[gene]['line'] + '\t' + str(min(test_dict[gene]['pval'] * m_eff, 1)) + '\t' + str(m_eff) + '\t' + str(M) +'\n')
            OUT.flush()
            gc.collect()
...
def lw_shrink(genotypes):
    ...
    try:
        ...
        shrunk_precision = np.diag(np.diag(shrunk_cov) ** (-0.5))
        shrunk_cor = np.dot(np.dot(shrunk_precision, shrunk_cov), shrunk_precision)
    except:  # Exception for handling case where SNPs in the window are all in perfect LD
        row = np.ones(m)
        shrunk_cor = np.outer(row, row)
        alpha = 'NA'
...
parser.add_argument('--var_thresh', dest='var_thresh', default=0.99, type=float, help='variance threshold')
parser.add_argument('--OUT', dest='OUT', required=True, help='output filename')
parser.add_argument('--window', dest='window', default=200, type=int, help='SNP window size')
...
parser.add_argument('--cis_dist', dest='cis_dist', default=1e6, type=float, help='threshold for bp distance from the gene TSS to perform multiple testing correction (default = 1e6)')
